...
HAMZA_ERRORS: dict[str, str] = {
    ...
    "امام": "أمام", "امامه": "أمامه", "امامها": "أمامها", "امامي": "أمامي",
...
# نُدرِج نطاق التشكيل (\u064B-\u065F، \u0670) والتطويل (\u0640) ضمن حروف الكلمة
# حتى لا يُقسِّم الـ Regex كلمة واحدة مُشكَّلة (مثل: الشَّاطِئُ) إلى أجزاء عند
# كل علامة تشكيل (كانت المشكلة سابقاً: الشَّاطِئُ → "الش" + "اطئ" خطأً).
# التطبيع (إزالة التشكيل) يحدث لاحقاً في _normalize_for_lookup للمقارنة فقط،
# بينما start/end المُعادان للواجهة يبقيان يُشيران للكلمة الكاملة كما وردت.
_TOKEN_RE = re.compile(r"[\w\u0621-\u064A\u064B-\u065F\u0670\u0640]+", re.UNICODE)
